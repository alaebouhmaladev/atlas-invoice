<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        <div class="flex items-center gap-2 mb-1">
          ...
          <span class="text-xs text-brand font-bold">Sécurité</span>
        </div>
        <h1 class="text-2xl font-extrabold text-main tracking-tight">Sécurité du Compte & Sessions</h1>
        <p class="text-xs text-muted-custom mt-0.5">Modifiez votre mot de passe et gérez vos appareils et sessions actives.</p>
      </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="bg-panel border border-custom rounded-panel p-6 shadow-soft space-y-4">
        <h2 class="text-sm font-bold text-main border-b border-custom pb-2 flex items-center gap-2">
        ...
        <form @submit.prevent="handleChangePassword" class="space-y-3">
          <div>
            ...
            <input v-model="passForm.currentPassword" type="password" required placeholder="••••••••••••" class="w-full px-3.5 py-2 bg-panel-raised border border-custom rounded-control text-xs text-main focus:outline-none focus:border-brand" />
          </div>
          ...
          <div>
            ...
            <input v-model="passForm.confirmPassword" type="password" required placeholder="••••••••••••" class="w-full px-3.5 py-2 bg-panel-raised border border-custom rounded-control text-xs text-main focus:outline-none focus:border-brand" />
          </div>
      ...
      <div class="bg-panel border border-custom rounded-panel p-6 shadow-soft space-y-4">
        <div class="flex items-center justify-between border-b border-custom pb-2">
          <h2 class="text-sm font-bold text-main flex items-center gap-2">
