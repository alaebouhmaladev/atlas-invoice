      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="p-5 bg-panel border border-custom rounded-card shadow-soft space-y-3">
          <div class="flex items-center justify-between">
            <div class="w-9 h-9 rounded-control bg-brand-soft border border-brand-soft text-brand-strong font-bold flex items-center justify-center text-sm">
              AB
            </div>
            <span class="px-2.5 py-1 rounded-pill text-xs font-bold bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
              v{{ identity.version }}
            </span>
          </div>
          <div>
            <h3 class="text-sm font-bold text-main">{{ identity.productName }}</h3>
            <p class="text-xs text-muted-custom mt-0.5">{{ identity.description }}</p>
          </div>
          <div class="pt-2 border-t border-custom flex items-center justify-between text-xs text-muted-custom">
        ...
        <NuxtLink
          ...
          class="p-5 bg-panel hover:bg-surface-hover border border-custom hover:border-brand/40 rounded-card transition-all group shadow-soft flex flex-col justify-between"
        >
          <div>
            ...
            <h3 class="text-sm font-bold text-main group-hover:text-brand transition-colors">Sécurité & Sessions</h3>
            <p class="text-xs text-muted-custom mt-1">Mots de passe, révocations de sessions et journal d'audit de sécurité.</p>
          </div>
          <div class="text-xs text-brand-strong font-bold flex items-center gap-1 mt-3">
        ...
        <div class="p-5 bg-panel border border-custom rounded-card shadow-soft flex flex-col justify-between">
          <div>
            ...
            <p class="text-xs text-muted-custom mt-1">Sauvegardes PostgreSQL automatisées et état système via le CLI <code class="text-brand-strong bg-panel-raised px-1 py-0.5 rounded-control font-mono">atlascrm status</code>.</p>
          </div>
          <div class="pt-2 border-t border-custom flex items-center justify-between text-xs text-muted-custom mt-3">
    ...
    <div class="space-y-4">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <NuxtLink
          ...
          class="p-5 bg-panel hover:bg-surface-hover border border-custom hover:border-brand/40 rounded-card transition-all group shadow-soft"
        >
          <div class="w-9 h-9 rounded-control bg-brand-soft border border-brand-soft text-brand-strong flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <h3 class="text-sm font-bold text-main group-hover:text-brand transition-colors">Fiche Entreprise</h3>
          <p class="text-xs text-muted-custom mt-1">Raison sociale légale, identifiants fiscaux (ICE, IF, RC, CNSS), adresse et coordonnées bancaires.</p>
        </NuxtLink>
        ...
        >
          ...
          <h3 class="text-sm font-bold text-main group-hover:text-brand transition-colors">Documents & Actifs</h3>
          <p class="text-xs text-muted-custom mt-1">Logo officiel, signature, cachet, préfixes de numérotation et paramètres PDF.</p>
        </NuxtLink>
        ...
        >
          ...
          <h3 class="text-sm font-bold text-main group-hover:text-brand transition-colors">Gestion des Utilisateurs</h3>
          <p class="text-xs text-muted-custom mt-1">Gestion des accès de l'équipe, attribution des rôles et réinitialisations.</p>
        </NuxtLink>
      </div>
    </div>
  </div>
