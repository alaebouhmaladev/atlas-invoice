<template>
  <div class="space-y-6 max-w-4xl mx-auto">
    ...
    <div class="flex items-center justify-between">
      <div>
        <NuxtLink :to="`/rh/employes/${route.params.id}`" class="text-xs text-amber-400 hover:text-amber-300 font-semibold flex items-center gap-1 mb-2">
          ← Annuler et revenir à la fiche
        </NuxtLink>
        <h1 class="text-2xl font-bold text-slate-100 tracking-tight">Modifier le collaborateur</h1>
        <p class="text-xs text-slate-400 mt-1">Mise à jour des informations administratives et coordonnées RH</p>
      </div>
    </div>
