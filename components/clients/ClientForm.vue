  ...
  <form @submit.prevent="handleSubmit" class="space-y-8">
    ...
    <div class="bg-panel border border-custom rounded-panel p-6 space-y-5 shadow-soft">
      <div class="border-b border-custom pb-3">
        ...
        <p class="text-xs text-muted-custom mt-1">Sélectionnez le type de client et saisissez son nom légal.</p>
      </div>

      <!-- Type Switcher -->
      <div>
        <label class="block text-xs font-bold text-muted-custom uppercase tracking-wider mb-2">Type de client *</label>
        <div class="grid grid-cols-2 gap-3 max-w-md">
          <button
            type="button"
            @click="form.type = 'COMPANY'"
            class="py-3 px-4 rounded-control border text-xs font-bold flex items-center justify-center gap-2 transition-all cursor-pointer"
            :class="form.type === 'COMPANY' ? 'bg-brand-soft border-brand-soft text-brand-strong' : 'bg-panel-raised border-custom text-muted-custom hover:text-main'"
          >
          ...
          <button
            ...
            class="py-3 px-4 rounded-control border text-xs font-bold flex items-center justify-center gap-2 transition-all cursor-pointer"
            :class="form.type === 'INDIVIDUAL' ? 'bg-brand-soft border-brand-soft text-brand-strong' : 'bg-panel-raised border-custom text-muted-custom hover:text-main'"
          >
            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div v-if="form.type === 'COMPANY'">
        <label for="companyName" class="block text-xs font-bold text-muted-custom uppercase tracking-wider mb-1">Raison sociale *</label>
        <input
          id="companyName"
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label for="firstName" class="block text-xs font-bold text-muted-custom uppercase tracking-wider mb-1">Prénom *</label>
          <input
            id="firstName"
        ...
        <div>
          <label for="lastName" class="block text-xs font-bold text-muted-custom uppercase tracking-wider mb-1">Nom *</label>
          <input
            id="lastName"
    ...
    <div v-if="form.type === 'COMPANY'" class="bg-panel border border-custom rounded-panel p-6 space-y-5 shadow-soft">
      <div class="border-b border-custom pb-3">
        <h3 class="text-sm font-bold text-main uppercase tracking-wider flex items-center gap-2">
        ...
        <p class="text-xs text-muted-custom mt-1">Identifiants obligatoires pour la conformité de facturation marocaine.</p>
      </div>

      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <!-- ICE (15 Digits) -->
        <div>
          <label for="ice" class="block text-xs font-bold text-muted-custom uppercase tracking-wider mb-1">
            ICE (15 chiffres)
          </label>
          <input
            id="ice"
        ...
        <div>
          <label for="taxId" class="block text-xs font-bold text-muted-custom uppercase tracking-wider mb-1">Identifiant Fiscal (IF)</label>
          <input
            id="taxId"
            v-model="form.taxId"
            type="text"
        ...
        <div>
          <label for="rc" class="block text-xs font-bold text-muted-custom uppercase tracking-wider mb-1">Registre du Commerce (RC)</label>
          <input
            id="rc"
            v-model="form.rc"
            type="text"
        ...
        <!-- CNSS -->
        <div>
          <label for="cnss" class="block text-xs font-bold text-muted-custom uppercase tracking-wider mb-1">N° CNSS</label>
          <input
            id="cnss"
            v-model="form.cnss"
            type="text"
        ...
        <!-- Patente -->
        <div>
          <label for="patent" class="block text-xs font-bold text-muted-custom uppercase tracking-wider mb-1">Patente</label>
          <input
            id="patent"
            v-model="form.patent"
            type="text"
    ...
    <div class="bg-panel border border-custom rounded-panel p-6 space-y-5 shadow-soft">
      <div class="border-b border-custom pb-3">
        <h3 class="text-sm font-bold text-main uppercase tracking-wider flex items-center gap-2">
          <span class="w-6 h-6 rounded-control bg-brand-soft text-brand-strong flex items-center justify-center text-xs font-bold border border-brand-soft">
            {{ form.type === 'COMPANY' ? 3 : 2 }}
          </span>
          Coordonnées & Adresse
        </h3>
        <p class="text-xs text-muted-custom mt-1">Adresse de facturation et numéros de contact direct.</p>
      </div>

      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label for="email" class="block text-xs font-bold text-muted-custom uppercase tracking-wider mb-1">Email principal</label>
          <input
            id="email"
            v-model="form.email"
            type="email"
        ...
        <div>
          <label for="phone" class="block text-xs font-bold text-muted-custom uppercase tracking-wider mb-1">Téléphone principal</label>
          <input
            id="phone"
            v-model="form.phone"
            type="tel"
        ...
        <div>
          <label for="secondaryPhone" class="block text-xs font-bold text-muted-custom uppercase tracking-wider mb-1">Téléphone secondaire</label>
          <input
            id="secondaryPhone"
        ...
        <div>
          <label for="city" class="block text-xs font-bold text-muted-custom uppercase tracking-wider mb-1">Ville</label>
          <input
            id="city"
            v-model="form.city"
            type="text"
        ...
        <div class="sm:col-span-2">
          <label for="address" class="block text-xs font-bold text-muted-custom uppercase tracking-wider mb-1">Adresse ligne 1</label>
          <input
            id="address"
        ...
        <div>
          <label for="addressLine2" class="block text-xs font-bold text-muted-custom uppercase tracking-wider mb-1">Complément d'adresse</label>
          <input
            id="addressLine2"
        ...
        <div>
          <label for="postalCode" class="block text-xs font-bold text-muted-custom uppercase tracking-wider mb-1">Code postal</label>
          <input
            id="postalCode"
    ...
    <div v-if="form.type === 'COMPANY'" class="bg-panel border border-custom rounded-panel p-6 space-y-5 shadow-soft">
      <div class="border-b border-custom pb-3">
        <h3 class="text-sm font-bold text-main uppercase tracking-wider flex items-center gap-2">
          <span class="w-6 h-6 rounded-control bg-brand-soft text-brand-strong flex items-center justify-center text-xs font-bold border border-brand-soft">4</span>
          Personne de Contact
        </h3>
        <p class="text-xs text-muted-custom mt-1">Interlocuteur privilégié pour les événements traiteur et devis.</p>
      </div>

      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label for="contactName" class="block text-xs font-bold text-muted-custom uppercase tracking-wider mb-1">Nom du contact</label>
          <input
            id="contactName"
        ...
        <div>
          <label for="contactPosition" class="block text-xs font-bold text-muted-custom uppercase tracking-wider mb-1">Fonction / Poste</label>
          <input
            id="contactPosition"
        ...
        <div>
          <label for="contactEmail" class="block text-xs font-bold text-muted-custom uppercase tracking-wider mb-1">Email du contact</label>
          <input
            id="contactEmail"
        ...
        <div>
          <label for="contactPhone" class="block text-xs font-bold text-muted-custom uppercase tracking-wider mb-1">Téléphone du contact</label>
          <input
            id="contactPhone"
    ...
    <div class="bg-panel border border-custom rounded-panel p-6 space-y-5 shadow-soft">
      <div class="border-b border-custom pb-3">
        ...
        <p class="text-xs text-muted-custom mt-1">Préférences alimentaires, spécificités traiteur, réductions accordées.</p>
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          class="w-full px-4 py-3 bg-panel-raised border border-custom rounded-control text-xs text-main focus:outline-none focus:border-brand resize-y"
        ></textarea>
      </div>
    </div>
    ...
    <div class="flex items-center justify-end gap-4 pt-4">
      ...
      <button
        type="submit"
        :disabled="loading"
        class="px-6 py-2.5 bg-brand text-on-brand hover:bg-brand-strong font-bold rounded-pill text-xs shadow-sm focus:outline-none transition-all disabled:opacity-50 flex items-center gap-2 cursor-pointer"
      >
