<template>
  <Teleport to="body">
    ...
    >
      <div
        class="w-full max-w-lg bg-slate-900 border border-rose-500/30 rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden flex flex-col my-auto"
      >
        <!-- Modal Header -->
        <div class="px-5 py-4 bg-rose-500/10 border-b border-rose-500/20 flex items-center justify-between gap-3">
          <div class="flex items-center gap-3 min-w-0">
            ...
            <div class="truncate">
              ...
              <p class="text-xs text-rose-400 font-medium truncate">
        ...
        <div class="p-5 space-y-4 max-h-[75vh] overflow-y-auto">
          ...
          <div v-if="previewData" class="p-3.5 bg-slate-950 border border-slate-800 rounded-2xl space-y-2">
            <div class="text-xs font-semibold text-slate-300 mb-1">Résumé de la classification :</div>
            <div v-if="previewData.permanentDeleteAllowed > 0" class="text-xs text-rose-400 flex items-center gap-1.5 font-medium">
              <span class="w-1.5 h-1.5 rounded-full bg-rose-500"></span>
              <span><strong>{{ previewData.permanentDeleteAllowed }}</strong> brouillon(s) sera(seront) supprimé(s) définitivement.</span>
            </div>
            <div v-if="previewData.archiveOnly > 0" class="text-xs text-amber-400 flex items-center gap-1.5 font-medium">
              <span class="w-1.5 h-1.5 rounded-full bg-amber-500"></span>
              <span><strong>{{ previewData.archiveOnly }}</strong> document(s) officiel(s) sera(seront) archivé(s).</span>
            </div>
            <div v-if="previewData.blocked > 0" class="text-xs text-slate-400 flex items-center gap-1.5">
          ...
          <p class="text-xs text-slate-300 leading-relaxed bg-rose-500/5 border border-rose-500/10 p-3 rounded-xl">
          ...
          <div class="space-y-1.5">
            <label class="block text-xs font-semibold text-slate-200">
              Motif de l’action <span class="text-rose-400">*</span>
            </label>
            <input
              v-model="reasonText"
              type="text"
              placeholder="ex: Nettoyage administratif de fin de mois (min 10 caractères)"
              class="w-full px-3.5 py-2.5 bg-slate-950 border border-slate-800 focus:border-rose-500 text-slate-100 placeholder-slate-500 rounded-xl text-xs outline-none transition-colors"
            />
            <p v-if="reasonText.trim().length > 0 && reasonText.trim().length < 10" class="text-[11px] text-rose-400 font-medium">
          ...
          <div v-if="isReauthRequired" class="space-y-1.5 p-3.5 bg-amber-500/5 border border-amber-500/20 rounded-2xl">
            <label class="block text-xs font-semibold text-amber-300">
              Mot de passe de confirmation (ré-authentification requise) <span class="text-rose-400">*</span>
            </label>
            <input
              v-model="passwordInput"
              type="password"
              placeholder="Entrez votre mot de passe Super Admin"
              class="w-full px-3.5 py-2.5 bg-slate-950 border border-amber-500/30 focus:border-amber-400 text-slate-100 placeholder-slate-500 rounded-xl text-xs outline-none transition-colors"
            />
          ...
          <div class="space-y-1.5">
            <label class="block text-xs font-semibold text-slate-200">
              Veuillez saisir exactement <code class="px-1.5 py-0.5 bg-slate-950 text-rose-400 border border-rose-500/20 rounded text-[11px] font-mono select-all">{{ requiredPhrase }}</code> pour confirmer :
            </label>
            <input
              v-model="typedPhrase"
              type="text"
              :placeholder="requiredPhrase"
              class="w-full px-3.5 py-2.5 bg-slate-950 border border-slate-800 focus:border-rose-500 text-slate-100 placeholder-slate-600 rounded-xl text-xs font-mono outline-none transition-colors"
            />
          </div>
        </div>

        <!-- Modal Actions Footer -->
        <div class="px-5 py-3.5 bg-slate-950 border-t border-slate-800 flex items-center justify-end gap-3 shrink-0">
          <button
            type="button"
            @click="close"
          ...
          <button
            ...
            @click="confirm"
            class="px-4 py-2 rounded-xl text-xs font-bold transition-colors inline-flex items-center gap-2 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
            :class="actionType === 'RESTORE' ? 'bg-amber-500 hover:bg-amber-400 text-slate-950' : 'bg-rose-600 hover:bg-rose-500 text-white'"
          >
